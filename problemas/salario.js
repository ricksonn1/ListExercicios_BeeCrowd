...
// um espaço em branco antes e depois da igualdade. No caso do salário, também deve haver um espaço em branco após o $.


const prompt = require('prompt-sync')()

const numeroDoFuncionario = Number(prompt('Digite seu número de funcionário: '));
const numeroDeHorasTrabalhadas = Number(prompt('Quantas horas você trabalha no mês? '))
const valorQueRecebePorHora = Number(prompt('Qual valor você recebe por hora? '))

const salário = numeroDeHorasTrabalhadas * valorQueRecebePorHora;

console.log(`Número do funcionário = ${numeroDoFuncionario}` + `\nSalário = R$ ${salário.toFixed(2)}`)